...
class LinesorterAPIClientError(Exception):
...
class ValidationError(LinesorterAPIClientError):
...
class LinesorterAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"lines": {"type": "array", "required": True}, "order": {"type": "string", "required": False, "default": "asc"}, "caseSensitive": {"type": "boolean", "required": False, "default": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the LinesorterAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises LinesorterAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/linesorter'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise LinesorterAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[LinesorterAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        :raises LinesorterAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise LinesorterAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise LinesorterAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise LinesorterAPIClientError(f"Request failed: {str(e)}")
